...
function parseBannerMarkings(banner) {
  ...
  const cat4_regex = /COMINT|SI|-GAMMA|\/|TALENT\s*KEYHOLE|SI-G|SI-G-ECI\s[A-Z]{3}|SI-[A-Z]{3}|TK|HCS|GCS/gi;
  const cat7_regex =
    /ORIGINATOR\s*CONTROLLED|ORCON|NOT\s*RELEASABLE\s*TO\s*FOREIGN\s*NATIONALS|NOFORN|AUTHORIZED\s*FOR\s*RELEASE\s*TO\s*((USA|AUS|NZL)(,)?( *))*|REL\s*TO\s*((USA|AUS|NZL)(,)?( *))*|CAUTION-PROPERIETARY\s*INFORMATION\s*INVOLVED|PROPIN|RELIDO|FOR\s*OFFICIAL\s*USE\s*ONLY/gi;
  const cat4_and_cat7 =
    /COMINT|-GAMMA|\/|TALENT\s*KEYHOLE|SI-G|SI-G-ECI\s[A-Z]{3}|SI-[A-Z]{3}|TK|HCS|GCS|ORIGINATOR\s*CONTROLLED|ORCON|NOT\s*RELEASABLE\s*TO\s*FOREIGN\s*NATIONALS|NOFORN|AUTHORIZED\s*FOR\s*RELEASE\s*TO\s*((USA|AUS|NZL)(,)?( *))*|REL\s*TO\s*((USA|AUS|NZL)(,)?( *))*|CAUTION-PROPERIETARY\s*INFORMATION\s*INVOLVED|PROPIN/gi;
